                ...
                <?php 
                    ...
                    if($select_order->rowCount() > 0) {
                        while($fetch_order = $select_order->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $fetch_product->execute([$fetch_order['product_id']]);
                            if($fetch_product->rowCount() > 0) {
                                while($fetch_p = $fetch_product->fetch(PDO::FETCH_ASSOC)) {
                ?>

                <div class="box">
                    <div class="status" style="color: <?php if($fetch_order['status'] == 'in progress') {echo "limegreen";}
                    else {echo "coral";} ?>"><?= $fetch_order['status']; ?></div>

                    <div class="detail">
                        <p>user name: <span><?= $fetch_order['name']; ?></span></p>
                        <p>Tên sản phẩm <span><?= $fetch_p['name']; ?></span></p>
                        ...
                        <p>total price: <span><?= $fetch_order['price']*$fetch_order['qty']; ?></span></p>
